...
async function main() {
  ...
  let proxyAddress = OUTPUT_DEPLOY[network.name]["CINEX"].proxyAddress;

  if (proxyAddress.length == 0) {
    console.error("Invalid existing proxy address!");
    process.exit(1);
  }
  console.assert(proxyAddress.length > 0, "Invalid existing proxy address!");
  ...
  try {
    ...
    newImplAddress = await upgrades.prepareUpgrade(proxyAddress, newImpl, {
      kind: "uups",
    });
  ...
  OUTPUT_DEPLOY[network.name]["CINEX"].implAddress = newImplAddress;
