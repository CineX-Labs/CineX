...
describe("CINEX", function () {
  async function deployFixture() {
    ...
    const [owner, otherAccount, dexPool] = await ethers.getSigners();

    const liquidity = await ethers.getImpersonatedSigner(
      "0x2c0e66A4Fe460eA18d0887B7849fe0395c15f9D3",
    );
    const debtManagement = await ethers.getImpersonatedSigner(
      "0x58C498f8eFbC7Cc1c7299FE7C94F0A0836BB1034",
    );
    const acquisition = await ethers.getImpersonatedSigner(
      "0x1331b032029e0a38BFE56d287B4befE64D04D0c6",
    );
    const development = await ethers.getImpersonatedSigner(
      "0x8b8702A4266F80f270B45C8775F0335C042E48BF",
    );
    const community = await ethers.getImpersonatedSigner(
      "0x9aD19B438c19cb3d32C23a4b9010f432d3fC94e3",
    );
    const reserve = await ethers.getImpersonatedSigner(
      "0x80657b98772CB75DDEceF1bB92647FDca3e1a0A1",
    );
    const marketing = await ethers.getImpersonatedSigner(
      "0x94baA3A22778dfAbeC37Cc379E51B43A255d9c5E",
    );
    const team = await ethers.getImpersonatedSigner(
      "0xe813B4588c93B0DA5146314C179b67e6c7690894",
    );

    await owner.sendTransaction({
      to: liquidity.address,
      value: ethers.parseEther("1"),
    });
    await owner.sendTransaction({
      to: debtManagement.address,
      value: ethers.parseEther("1"),
    });
    await owner.sendTransaction({
      to: acquisition.address,
      value: ethers.parseEther("1"),
    });
    await owner.sendTransaction({
      to: development.address,
      value: ethers.parseEther("1"),
    });
    await owner.sendTransaction({
  ...
  describe("Deployment", function () {
    it("Should set the right args", async function () {
      ...
      expect(await token.INITIAL_SUPPLY()).to.be.equal(
      ...
      expect(await token.debtManagementWallet()).to.be.equal(
        debtManagement.address,
      );
      ...
      expect(await token.swapFeeChangeTime()).to.be.closeTo(
        deployTime + 60 * 60 * 24 * 365,
        5,
      );
      expect(await token.removeTransferRestrictionTime()).to.be.closeTo(
        deployTime + 60 * 60 * 24 * 60,
        5,
      );
    });
    ...
    it("Should mint and distribute the right amounts", async function () {
      ...
      expect(await token.totalSupply()).to.be.equal(
        (ethers.parseEther("1000000000") * BigInt(90)) / BigInt(100),
      );
      expect(await token.balanceOf(liquidity.address)).to.be.equal(
        (ethers.parseEther("1000000000") * BigInt(15)) / BigInt(100),
      );
      expect(await token.balanceOf(debtManagement.address)).to.be.equal(
        (ethers.parseEther("1000000000") * BigInt(27)) / BigInt(100),
      );
      expect(await token.balanceOf(acquisition.address)).to.be.equal(
        (ethers.parseEther("1000000000") * BigInt(23)) / BigInt(100),
      );
      expect(await token.balanceOf(development.address)).to.be.equal(
        (ethers.parseEther("1000000000") * BigInt(5)) / BigInt(100),
      );
      expect(await token.balanceOf(community.address)).to.be.equal(
        (ethers.parseEther("1000000000") * BigInt(5)) / BigInt(100),
      );
      expect(await token.balanceOf(reserve.address)).to.be.equal(
        (ethers.parseEther("1000000000") * BigInt(5)) / BigInt(100),
      );
      expect(await token.balanceOf(marketing.address)).to.be.equal(
        (ethers.parseEther("1000000000") * BigInt(5)) / BigInt(100),
      );
      expect(await token.balanceOf(team.address)).to.be.equal(
        (ethers.parseEther("1000000000") * BigInt(5)) / BigInt(100),
      );
    });
  ...
  describe("Swap fees", function () {
    ...
    describe("Setters", function () {
      it("Should set the fee free list", async function () {
        ...
        await expect(token.setFeeFreeList(owner.address, true))
          .to.be.emit(token, "FeeFreeListUpdated")
          .withArgs(owner.address, true);

        expect(await token.isFeeFree(owner.address)).to.be.true;

        await expect(token.setFeeFreeList(owner.address, false))
          .to.be.emit(token, "FeeFreeListUpdated")
      ...
      it("Should set the transfer restriction free list", async function () {
        ...
        expect(await token.isTransferRestrictionFree(owner.address)).to.be
          .false;

        await expect(token.setTransferRestrictionFreeList(owner.address, true))
          .to.be.emit(token, "TransferRestrictionFreeListUpdated")
          .withArgs(owner.address, true);

        expect(await token.isTransferRestrictionFree(owner.address)).to.be.true;

        await expect(token.setTransferRestrictionFreeList(owner.address, false))
          .to.be.emit(token, "TransferRestrictionFreeListUpdated")
          .withArgs(owner.address, false);
      ...
      it("Should set the pool with fee list", async function () {
        ...
        await expect(token.setPoolWithFeeList(dexPool.address, true))
          .to.be.emit(token, "PoolWithFeeListUpdated")
          .withArgs(dexPool.address, true);

        expect(await token.isPoolWithFee(dexPool.address)).to.be.true;

        await expect(token.setPoolWithFeeList(dexPool.address, false))
          .to.be.emit(token, "PoolWithFeeListUpdated")
          .withArgs(dexPool.address, false);
      ...
      it("Should revert set the fee free list with zero address", async function () {
        ...
        await expect(
          token.setFeeFreeList(ethers.ZeroAddress, true),
      ...
      it("Should revert set the transfer restriction free list with zero address", async function () {
        ...
        expect(await token.isTransferRestrictionFree(ethers.ZeroAddress)).to.be
          .false;

        await expect(
          token.setTransferRestrictionFreeList(ethers.ZeroAddress, true),
        ).to.be.revertedWithCustomError(token, "ZeroAddress");
      ...
      it("Should revert set the fee free list by not owner", async function () {
        ...
        await expect(
          token.connect(otherAccount).setFeeFreeList(owner.address, true),
      ...
      it("Should revert set the transfer restriction free list by not owner", async function () {
        ...
        await expect(
          token
            ...
            .setTransferRestrictionFreeList(owner.address, true),
        ).to.be.revertedWithCustomError(token, "OwnableUnauthorizedAccount");
      ...
      it("Should revert set the pool with fee list with zero address", async function () {
        ...
        await expect(
          token.setPoolWithFeeList(ethers.ZeroAddress, true),
        ).to.be.revertedWithCustomError(token, "ZeroAddress");
      ...
      it("Should revert set the pool with fee list by not owner", async function () {
        ...
        await expect(
          token.connect(otherAccount).setPoolWithFeeList(owner.address, true),
        ).to.be.revertedWithCustomError(token, "OwnableUnauthorizedAccount");
    ...
    describe("Get swap fee", function () {
      it("Should take and distribute swap fee on sell before update fee time", async function () {
        const { token, liquidity, development, reserve, dexPool } =
          await loadFixture(deployFixture);
        ...
        const expectedLiquidityPart =
          (transferAmount * BigInt(4)) / BigInt(100);
        const expectedDevelopmentPart =
          (transferAmount * BigInt(2)) / BigInt(100);

        await expect(
          token.connect(reserve).transfer(dexPool.address, transferAmount),
        ).to.be.changeTokenBalances(
          token,
          [reserve, dexPool, liquidity, development],
          [
            -transferAmount,
      ...
      it("Should take and distribute swap fee on sell after update fee time", async function () {
        ...
        const expectedLiquidityPart =
          (transferAmount * BigInt(1)) / BigInt(100);
        const expectedDevelopmentPart =
          (transferAmount * BigInt(1)) / BigInt(100);
      ...
      it("Should take and distribute swap fee on buy before update fee time", async function () {
        ...
        await expect(
          token.connect(dexPool).transfer(reserve.address, transferAmount),
        ).to.be.changeTokenBalances(
          token,
          [reserve, dexPool, liquidity, development],
          [
    ...
    describe("Antibot cooldown", function () {
      it("Should revert if less than 30 seconds have passed from the last swap", async function () {
        ...
        await expect(
          token.connect(reserve).transfer(dexPool.address, transferAmount),
        ).to.be.revertedWithCustomError(token, "AntibotCooldown");
      });
  ...
  describe("Transfer limit", function () {
    it("Should revert if exeeds transfer limit", async function () {
      ...
      const transferAmount =
        (await token.INITIAL_SUPPLY()) / BigInt(100) + BigInt(1);

      await expect(
        token.connect(liquidity).transfer(owner.address, transferAmount),
    ...
    it("Should disable transfer limit after 60 days", async function () {
      ...
      await expect(
        ...
        .withArgs(liquidity.address, owner.address, transferAmount);
    });
    ...
    it("Should disable transfer limit for user from transfer restriction free", async function () {
      ...
      await expect(token.transfer(liquidity.address, transferLimit + BigInt(1)))
        .to.be.emit(token, "Transfer")
        .withArgs(owner.address, liquidity.address, transferLimit + BigInt(1));
    });
  });
  ...
  describe("Transfer from", function () {
    it("Should transfer tokens", async function () {
      ...
      await expect(
        token
          .connect(owner)
          .transferFrom(reserve.address, owner.address, transferAmount),
      ).to.be.changeTokenBalances(
        token,
        [reserve, owner],
    ...
    it("Should revert transfer tokens if insufficient allowance", async function () {
      ...
      await token
        .connect(reserve)
        .approve(owner.address, transferAmount - BigInt(1));
  ...
  describe("Pauseable", function () {
    ...
    it("Should revert unpause by not owner", async function () {
      ...
      await expect(
        token.connect(otherAccount).unpause(),
      ).to.be.revertedWithCustomError(token, "OwnableUnauthorizedAccount");
    ...
    it("Should revert any transfer on pause", async function () {
      ...
      await expect(
        token.transferFrom(liquidity.address, owner.address, transferAmount),
      ).to.be.revertedWithCustomError(token, "EnforcedPause");
    });
  });
  ...
  describe("UUPSUpgradeable", function () {
    it("Should upgrade", async function () {
      ...
      const tokenWithNewInterface = await ethers.getContractAt(
        "MockCINEXV2",
